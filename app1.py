...
@st.cache_data
def load_data():
    df = pd.read_csv("F:/Python_WC/Amazon Music Clustering/Amazon_Music_Clustering/amazon_music_final_clusters.csv")
    summary = pd.read_csv("F:/Python_WC/Amazon Music Clustering/Amazon_Music_Clustering/amazon_music_final_summary.csv")
    return df, summary
...
data = pd.read_csv("F:/Python_WC/Amazon Music Clustering/Amazon_Music_Clustering/cluster_interpretation_summary.csv")
st.dataframe(data)
...
plt.scatter(x=X_pca[:,0],y=X_pca[:,1], c=df['cluster'], cmap='tab10', alpha=0.6, edgecolor='k') 
...
st.markdown("---")
st.caption("Amazon Music Clustering | Unsupervised ML Project")